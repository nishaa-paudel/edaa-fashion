    ...
    <div class="your-order-table table-responsive">
        <table>
            ...
            @foreach($cartItems as $item)
                <tr class="cart_item">
                    ...
                    <td class="product-total">
                        <span class="amount">Rs. {{Cart::session(auth()->id())->get($item->id)->getPriceSum()}}</span>
                    </td>
                </tr>
            @endforeach
            </tbody>
            ...
            <tr class="cart-subtotal">
                <th>Cart Subtotal</th>
                <td><span class="amount">Rs. {{ auth()->user()->orderSubTotal() }}</span></td>
            </tr>
            <tr class="cart-subtotal">
                <th>Tax(13%)</th>
                <td><span class="amount">Rs. {{ auth()->user()->tax() }}</span></td>
            </tr>
            <tr class="order-total">
                <th>Order Quantity</th>
                <td><strong><span class="amount">Rs. {{ auth()->user()->orderTotal() }}</span></strong></td>
            </tr>
